...
#$ -o /cbica/projects/pncNmf/grmpyNMF/n229_longitudinal/results/logs/extractBasesMT.$JOB_ID.stdout
#### save the standard error:
#$ -e /cbica/projects/pncNmf/grmpyNMF/n229_longitudinal/results/logs/extractBasesMT.$JOB_ID.stderr
####
#### My email address:
...
if [ $? != 0 ] ; then
	...
	echo "${date_info} : Failure to execute extractBasesMT, JobID: ${JOB_ID}, method: ${method}, number of bases: ${num_bases}" >> /cbica/projects/pncNmf/grmpyNMF/n229_longitudinal/scripts/FailedExtractBasesExperimentsMT.txt
	exit 1
fi
