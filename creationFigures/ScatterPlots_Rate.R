...
NmfModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ TP1ageAtScan1 + sex + DeltaManualRating + ari_log, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
Limbic<-ggplot() +
  ...
  labs(x = "Irritability (log(ARI+1))", y = "Annual Percent Change in CT for Network 18 (%)") +
  theme(axis.title=element_text(size=20,face="bold"), axis.text=element_text(size=14), axis.title.x=element_text(color = "black"), axis.title.y=element_text(color = "black"))
  
### Save Scatterplot ###

ggsave(file="/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/creationFigures/Rate_COMBAT_NMF/ScatterPlot_ctRATE_Network18.png")
...
Limbic<-ggplot() +
  ...
  annotate("text",x = -Inf, y = Inf, hjust = -0.1,vjust = 1,label = p_text, parse=TRUE,size = 8, colour = "black",fontface ="italic" ) +
  theme(legend.position = "none") +
  labs(x = "Irritability (log(ARI+1))", y = "Annual Percent Change in CT for Network 1 (%)") +
  theme(axis.title=element_text(size=20,face="bold"), axis.text=element_text(size=14), axis.title.x=element_text(color = "black"), axis.title.y=element_text(color = "black"))

### Save Scatterplot ###

ggsave(file="/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/creationFigures/Rate_COMBAT_NMF/ScatterPlot_ctRATE_Network1.png")
