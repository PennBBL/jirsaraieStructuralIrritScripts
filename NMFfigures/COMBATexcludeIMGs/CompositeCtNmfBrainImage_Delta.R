...
img<-antsImageRead('/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/NMFcreation/MaskedByNeighborThresh400_WarpedToMNI/n281_24NmfComponentsMerged.nii.gz',4)
...
output<-read.csv("/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/n137_Delta_NMF/n137_rds_n137_Demo+ARI+QA+TP1CT_20180401_inclusion_DeltaEXCLUDE_ROI_n137_Nmf24BasesCT_Delta/gam_formula_DeltaAge_TP1ageAtScan1_ari_log_DeltaManualRating_sex_tp1CT_coefficients.csv")
...
tvalVoxVector[which(tvalVoxVector %in% c(5,12,10,19,23,3,24,16,4,14,2,6))]<-0
...
tvalVoxVector[which(tvalVoxVector==1)] <- -4.79
tvalVoxVector[which(tvalVoxVector==11)] <- -4.87
tvalVoxVector[which(tvalVoxVector==9)] <- -3.36
tvalVoxVector[which(tvalVoxVector==8)] <- -2.73
tvalVoxVector[which(tvalVoxVector==13)] <- -2.90
tvalVoxVector[which(tvalVoxVector==15)] <- -2.82
tvalVoxVector[which(tvalVoxVector==20)] <- -2.75
tvalVoxVector[which(tvalVoxVector==22)] <- -2.63
tvalVoxVector[which(tvalVoxVector==18)] <- -2.60
tvalVoxVector[which(tvalVoxVector==7)] <- -2.39
tvalVoxVector[which(tvalVoxVector==17)] <- -2.41
tvalVoxVector[which(tvalVoxVector==21)] <- -2.35
...
antsImageWrite(tvalMap,"/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/NMFcreation/finalFigures/n137_CtNmfSigComponents_t400_Delta.nii.gz")
