...
output<-read.csv("/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/n137_Rate_NMF/n137_rds_n137_Demo+ARI+QA_20180401_inclusion_DeltaEXCLUDE_ROI_n137_Nmf24BasesCT_Rate/gam_formula_TP1ageAtScan1_ari_log_DeltaManualRating_sex_coefficients.csv")
...
tvalVoxVector[which(tvalVoxVector %in% c(22,18,21,13,10,5,12,24,19,23,2,16,3,6,14,4))]<-0
...
tvalVoxVector[which(tvalVoxVector==1)] <- -4.16
tvalVoxVector[which(tvalVoxVector==11)] <- -3.90
tvalVoxVector[which(tvalVoxVector==9)] <- -3.15
tvalVoxVector[which(tvalVoxVector==20)] <- -2.83
tvalVoxVector[which(tvalVoxVector==7)] <- -2.50
tvalVoxVector[which(tvalVoxVector==8)] <- -2.53
tvalVoxVector[which(tvalVoxVector==15)] <- -2.66
tvalVoxVector[which(tvalVoxVector==17)] <- -2.57
...
antsImageWrite(tvalMap,"/data/jux/BBL/projects/jirsaraieStructuralIrrit/output/NMFcreation/finalFigures/n137_CtNmfSigComponents_t400_Rate.nii.gz")
