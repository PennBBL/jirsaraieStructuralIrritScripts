...
################################################################################
##### Remake the RDS Delta Spreadsheet to Include The COMBAT Values of TP1 #####
################################################################################
...
DeltaTP1<-read.csv("/data/jux/BBL/projects/jirsaraieStructuralIrrit/data/n140_TP1_NMF/n140_Nmf24BasesCT_COMBAT_TP1.csv")
RDS<-merge(RDS,DeltaTP1, by=c("bblid"))
colnames(RDS)[8]<-"scanid"
RDS$scanid.y<-NULL
rename<- sub("Ct", "TP1_Ct", names(RDS))
RDS<-rename.vars(RDS,names(RDS),rename)
saveRDS(RDS, "/data/jux/BBL/projects/jirsaraieStructuralIrrit/data/n137_Delta_NMF/n137_Demo+ARI+QA+TP1CT_COMBAT_20180401.rds")
